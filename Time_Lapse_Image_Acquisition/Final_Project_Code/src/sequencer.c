...
void *Sequencer(void *threadp)
{
	...
	execution_time_seq = (double *)malloc((frame_count + EXTRA_FRAMES)*sizeof(double));
	if(execution_time_seq == NULL)
	{
		printf("Mem allocation failed for EXECUTION_TIME_SEQ\n");
	}
	start_time_seq = (double *)malloc((frame_count + EXTRA_FRAMES)*sizeof(double));
	if(start_time_seq == NULL)
        {
                printf("Mem allocation failed for START_TIME_SEQ\n");
        }
	end_time_seq = (double *)malloc((frame_count + EXTRA_FRAMES)*sizeof(double));
	...
	printf("Press ANY key to start image capturing\n");
	getchar();	//Gunshot approach to start capturing frames
	...
	if(freq_img_capture == 1)
	{
		...
		do
		{
			...
			if(((start_time_seq + seqCnt) >= start_time_seq) && ((start_time_seq + seqCnt) <= (start_time_seq + (frame_count+EXTRA_FRAMES))))
			{
			...
			else
			{
				...
				syslog(LOG_INFO,"[SEQUENCER]: Invalid Data address");
			}
		...
		} while(!abortTest && (seqCnt < (frame_count + EXTRA_FRAMES)));
