...
static void mainloop(void)
{
	...
	if(deadline_calc)
	{
		...
		if(jitter > 0)
                {
                        fprintf(fptr,"\n%d,%lf,%lf,%d,%lf",frame_number,wcet,jitter,0,total_jitter);
                }
                if(jitter < 0)
                {
                        fprintf(fptr,"\n%d,%lf,%d,%lf,%lf",frame_number,wcet,0,jitter,total_jitter);
                }

	//	fprintf(fptr,"\n%d,%lf,%lf,%lf,%lf",frame_number,wcet,positive_jitter,negative_jitter,total_jitter);
...
/*@Function Name: *BnW_Image_Transform
 *@Brief        : Thread which carries out BNW transform on the image
 *@Param in [1] : void *threadp (thread_id)
 *@Return       : void
 * */
void *BnW_Image_Transform(void *threadp)
{
	threadParams_t *threadParams = (threadParams_t *)threadp;
	printf("/***********************ANALYSIS*******************************/\n");
	printf("Transformation :Black & White \n");
	printf("Please wait while analysis is being carried out................\n");
	mainloop();
	deadline_calc = 1;
	mainloop();
...
/*@Function Name: *Grayscale_Image_Transform
 *@Brief        : Thread which carries out grayscale transform on the image
 *@Param in [1] : void *threadp (thread_id)
 *@Return       : void
 * */
void *GrayScale_Image_Transform(void *threadp)
{
	threadParams_t *threadParams = (threadParams_t *)threadp;
	printf("/***********************ANALYSIS*******************************/\n");
        printf("Transformation :Grayscale \n");
	printf("Please wait while analysis is being carried out................\n");
	mainloop();
	deadline_calc = 1;
	mainloop();
...
int main(int argc,char **argv)
{
	if(argc >1)
	{
		...
		strncpy(hres_str,argv[4],sizeof(hres_str));
		strncpy(vres_str,argv[5],sizeof(vres_str));
	...
	{
		printf("USAGE: [./<exe name> | /dev/video0 | <frame_count> | <sharpen/ bnw / grayscale> | HRES | VRES]\n");
		exit(-1);
	}
	...
	fprintf(fptr,"Sr.No,Time_Per_Frame,Positive Jitter,Negative Jitter,Total Jitter");
	...
	if(strcmp(transformation,"sharpen") == 0)
        ...
        else if (strcmp(transformation,"bnw") == 0)
        {
    		...
    		rc=pthread_create(&threads[0],             // pointer to thread descriptor
                      		&rt_sched_attr[0],         // use specific attributes
                      		//(void *)0,               // default attributes
                      		BnW_Image_Transform,       // thread function entry point
                      		(void *)&(threadParams[0]) // parameters to pass in
                     );
		...
		// Grayscale Image
                rc=pthread_create(&threads[0],             // pointer to thread descriptor
                      		&rt_sched_attr[0],         // use specific attributes
                      		//(void *)0,               // default attributes
                      		GrayScale_Image_Transform, // thread function entry point
                      		(void *)&(threadParams[0]) // parameters to pass in
                     );
